...
plasticsLP <- read.csv("~/Sample-microplastics/Subsampling from a group of samples/Lorenz_plankton.csv",header=T,stringsAsFactors=F)
	plasticsLP$Morphology[which(plasticsLP$Morphology=="fiber ")] <- "fiber"
	plasticsLP$Color[which(plasticsLP$Color=="black ")] <- "black"
	plasticsLP$Color[which(plasticsLP$Color=="")] <- "transparent"	#there are three particles with no color;  set to transparent
plasticsLS <- read.csv("~/Sample-microplastics/Subsampling from a group of samples/Lorenz_sediment.csv",header=T,stringsAsFactors=F)
	plasticsLS$Morphology[which(plasticsLP$Morphology=="")] <- "fragment"	#there are three particles with no morphology;  set to fragment for purposes of this analysis
plasticsBB <- read.csv("~/Sample-microplastics/Subsampling from a group of samples/Bourdages_qik_birds.csv",header=T,stringsAsFactors=F)
	plasticsBB$Morphology[which(plasticsBB$Morphology=="Fiber")] <- "Fibre"	
plasticsTB <- read.csv("~/Sample-microplastics/Subsampling from a group of samples/Teboul_birds.csv",header=T,stringsAsFactors=F)
	colnames(plasticsTB)[colnames(plasticsTB)=="Particletype"] <- "Particle.type"
	plasticsTB$Particle.type[which(plasticsTB$Particle.type=="HPDE")] <- "HDPE"	
plasticsCS <- read.csv("~/Sample-microplastics/Subsampling from a group of samples/CJ_sediment.csv",header=T,stringsAsFactors=F)
	plasticsCS$Color[which(plasticsCS$Color==" blue")] <- "blue"
	plasticsCS$Color[which(plasticsCS$Color=="multi")] <- "multicoloured"
plasticsCI <- read.csv("~/Sample-microplastics/Subsampling from a group of samples/CJ_deep_sea_inverts.csv",header=T,stringsAsFactors=F)
plasticsCIt <- read.csv("~/Sample-microplastics/Subsampling from a group of samples/CJ_deep_sea_inverts_time.csv",header=T,stringsAsFactors=F)
plasticsRG <- read.csv("~/Sample-microplastics/Subsampling from a group of samples/Rain_garden_combined.csv",header=T,stringsAsFactors=F)
...
plasticsVT <- read.csv("~/Sample-microplastics/Subsampling from a group of samples/Vidal_Tsukada_720particles_delete_notes.csv",header=T,stringsAsFactors=F)

datasets <- list(plasticsLP,plasticsLS,plasticsBB,plasticsTB,plasticsCS,plasticsCI,plasticsCIt,plasticsRG)
datasetsbtype <- list(plasticsLP,plasticsLS,plasticsBB,plasticsTB,plasticsCS,plasticsCI,plasticsCIt,plasticsRG,plasticsVT)
datnames <- c("lorenz plankton", "lorenz sediment","bourdages birds", "teboul birds","CJ sediment", "CJ inverts", "CJ inverts time", "rain garden")
...
data_titles <- c("Surface Water","Sediment", "Animal tissue","Animal tissue", "Sediment", "Animal tissue", "Animal tissue","Storm water")
databtype_titles <- c("Surface Water","Sediment", "Animal tissue","Animal tissue", "Sediment", "Animal tissue", "Animal tissue","Storm water","Sediment")
...
plasticsLsS <- read.csv("~/Sample-microplastics/Lusher_Sediment.csv",header=T,stringsAsFactors=F)
plasticsLM <- read.csv("~/Sample-microplastics/Lusher_Mussels.csv",header=T,stringsAsFactors=F)
plasticsBM <- read.csv("~/Sample-microplastics/Brate_Mussels.csv",header=T,stringsAsFactors=F) # I removed # and % in one polymer time by hand before R would read it in
...
datasetsI <- list(plasticsLF,plasticsLsS,plasticsLM,plasticsBM,plasticsP1,plasticsP2,plasticsP3,plasticsRG1,plasticsRG2,plasticsRG3)
...
data_titlesI <- c("Animal tissue","Sediment", "Animal tissue","Animal tissue", "Surface water", "Surface water", "Surface water", "Storm water", "Storm water","Storm water")
datnamesI <- c("LusherFish","LusherSediment","LusherMussels","BrateMussels","Primpke1","Primpke2","Primpke3","Raingarden1","Raingarden2","Raingarden3")
...
datasetsBI <- list(plasticsLF,plasticsLsS,plasticsLM,plasticsBM,plasticsP1,plasticsP2,plasticsP3,plasticsRG1,plasticsRG2,plasticsRG3,plasticsVT1,plasticsVT2)
...
data_titlesBI <- c("Animal tissue","Sediment", "Animal tissue","Animal tissue", "Surface water", "Surface water", "Surface water", "Storm water", "Storm water","Storm water","Sediment","Sediment")
datnamesBI <- c("LusherFish","LusherSediment","LusherMussels","BrateMussels","Primpke1","Primpke2","Primpke3","Raingarden1","Raingarden2","Raingarden3","VidalBeach1","VidalBeach2")
...
load("~/Sample-microplastics/Rand_str_dp_narrow.Rdata")
load("~/Sample-microplastics/Shp_str_dp_narrow.Rdata")
load("~/Sample-microplastics/Col_str_dp_narrow.Rdata")
load("~/Sample-microplastics/ShpCol_str_dp_narrow.Rdata")
...
load("~/Sample-microplastics/Rand_str_pool_narrow.Rdata")
load("~/Sample-microplastics/Shp_str_pool_narrow.Rdata")
load("~/Sample-microplastics/Col_str_pool_narrow.Rdata")
load("~/Sample-microplastics/ShpCol_str_pool_narrow.Rdata")
...
load("~/Sample-microplastics/Rand_str_dp_b.Rdata")
load("~/Sample-microplastics/Shp_str_dp_b.Rdata")
load("~/Sample-microplastics/Col_str_dp_b.Rdata")
load("~/Sample-microplastics/ShpCol_str_dp_b.Rdata")
...
load("~/Sample-microplastics/Rand_str_pool_b.Rdata")
load("~/Sample-microplastics/Shp_str_pool_b.Rdata")
load("~/Sample-microplastics/Col_str_pool_b.Rdata")
load("~/Sample-microplastics/ShpCol_str_pool_b.Rdata")
...
pdf("~/Sample-microplastics/Broad_all_panel_a_indSumE.pdf",height=3.2,width=8)
layout(matrix(c(1:12),ncol=6,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysizeBI){
plot(1~1, pch=NA, xlim=c(0,nrowsBI[i]),ylim=c(0,1),ylab="",xlab="")
	...
	if(i==7){mtext("Summed error",line=2.5,side=2,adj=3)}
	if(i==10){mtext("Particles chemically identified",line=2.5,side=1,adj=-0.05)}
	if(i==12){legend(150,y=0.65,c("20%Error"),lty=2,bty="n")}
	if(i==12){legend(150,y=1,c("Rand","Col","Morph","MorphCol"),fill=c(rgb(0,0,0),rgb(1,0,0),rgb(0,0,1),rgb(1,0,1)),bty="n")}
}
dev.off()
#
pdf("~/Sample-microplastics/Broad_all_panel_b_equalSumStrE.pdf",height=4.9,width=4.5)
...
plot(1~1, pch=NA, xlim=c(0,nrowsbtype[i]),ylim=c(0,1),ylab="",xlab="")
	...
	if(i==5){mtext("Summed error",line=2.5,side=2)}
	if(i==8){mtext("Particles chemically identified",line=2.5,side=1)}
	if(i==9){legend(50,y=0.65,c("20%Error"),lty=2,bty="n")}
}
dev.off()
#
pdf("~/Sample-microplastics/Broad_all_panel_c_pooledSumE.pdf",height=4.9,width=4.5)
layout(matrix(c(1:9),ncol=3,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysizeb){
plot(1~1, pch=NA, xlim=c(0,nrowsbtype[i]),ylim=c(0,1),ylab="",xlab="")
	...
	if(i==5){mtext("Summed error",line=2.5,side=2)}
	if(i==8){mtext("Particles chemically identified",line=2.5,side=1)}
	if(i==9){legend(150,y=1,c("Rand","Col","Morph","MorphCol"),fill=c(rgb(0,0,0),rgb(1,0,0),rgb(0,0,1),rgb(1,0,1)),bty="n")}
}
dev.off()

#####For NARROW material types
pdf("~/Sample-microplastics/Narrow_all_panel_a_indSumE.pdf",height=3.2,width=8)
layout(matrix(c(1:10,11,11),ncol=6,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysizeI){
...
plot(1~1, pch=NA, xlim=c(0,nrowsI[i]),ylim=c(0,1),ylab="",xlab="",bty="n",xaxt="n",yaxt="n")
	legend(250,y=0.65,c("20%Error"),lty=2,bty="n")
	legend(0,y=1,c("Rand","Col","Morph","MorphCol"),fill=c(rgb(0,0,0),rgb(1,0,0),rgb(0,0,1),rgb(1,0,1)),bty="n")
dev.off()
#
pdf("~/Sample-microplastics/Narrow_all_panel_b_equalSumStrE.pdf",height=4.9,width=4.5)
layout(matrix(c(1:9),ncol=3,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysize){
...
dev.off()
pdf("~/Sample-microplastics/Narrow_all_panel_c_poolSumE.pdf",height=4.9,width=4.5)
layout(matrix(c(1:9),ncol=3,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysize){
...
pdf("~/Sample-microplastics/Narrow_panel_a_indEstSampleRichness.pdf",height=3.2,width=8)
layout(matrix(c(1:10,11,11),ncol=6,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysizeI){
plot(1~1, pch=NA, xlim=c(0,nrowsI[i]),ylim=c(0,20),ylab="",xlab="")
	...
	if(i==7){mtext("# Material types observed",line=2.5,side=2,adj=1.1)}
	if(i==1){mtext("Particles chemically identified",line=2.5,side=1,adj=-0.05)}
}
plot(1~1, pch=NA, xlim=c(0,nrowsI[i]),ylim=c(0,1),ylab="",xlab="",bty="n",xaxt="n",yaxt="n")
...
dev.off()
#
pdf("~/Sample-microplastics/Narrow_panel_b_equalEstDatasetRichnes.pdf",height=4.9,width=4.5)
layout(matrix(c(1:9),ncol=3,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysize){
plot(1~1, pch=NA, xlim=c(0,nrows[i]),ylim=c(0,40),ylab="",xlab="")
	...
	mtext(data_titles[i], side=3, line =0.25, cex=1)
	if(i==5){mtext("# Material types observed",line=2.5,side=2)}
	if(i==8){mtext("Particles chemically identified",line=2.5,side=1)}
}
dev.off()
#
pdf("~/Sample-microplastics/Narrow_panel_c_poolEstDatasetRichness.pdf",height=4.9,width=4.5)
layout(matrix(c(1:9),ncol=3,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysize){
...
pdf("~/Sample-microplastics/Narrow_panel_a_indEstEnvRichness.pdf",height=3.2,width=8)
layout(matrix(c(1:10,11,11),ncol=6,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysizeI){
plot(1~1, pch=NA, xlim=c(0,nrowsI[i]),ylim=c(0,65),ylab="",xlab="")
	...
	mtext(data_titlesI[i], side=3, line =0.25, cex=1)
	if(i==7){mtext("# Material types estimated (Chao)",line=2.5,side=2,adj=1)}#using chao from subsample
	if(i==1){mtext("Particles chemically identified",line=2.5,side=1,adj=-0.05)}
}
plot(1~1, pch=NA, xlim=c(0,nrowsI[i]),ylim=c(0,1),ylab="",xlab="",bty="n",xaxt="n",yaxt="n")
	legend(-10,y=0.4,c("Full Sample/Dataset Chao Estimated #","Full Sample/Dataset # obs"),lty=3,lwd=1.5,bty="n",col=c(rgb(0,0,0),rgb(0.5,0.5,0.5) ) )
	legend(-10,y=1.1,c("Rand","Col","Morph","MorphCol"),fill=c(rgb(0,0,0),rgb(1,0,0),rgb(0,0,1),rgb(1,0,1)),bty="n")
dev.off()
#
pdf("~/Sample-microplastics/Narrow_panel_b_equalEstEnvRichness.pdf",height=4.9,width=4.5)
layout(matrix(c(1:9),ncol=3,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysize){
plot(1~1, pch=NA, xlim=c(0,nrows[i]),ylim=c(0,110),ylab="",xlab="")
	...
	abline(h=richness[i],lty=3,lwd=1.5,col=rgb(0.5,0.5,0.5))
	mtext(data_titles[i], side=3, line =0.25, cex=1)
	if(i==5){mtext("# Material types estimated (Chao)",line=2.5,side=2)}#using chao from subsample
	if(i==8){mtext("Particles chemically identified",line=2.5,side=1)}
}
dev.off()
#
pdf("~/Sample-microplastics/Narrow_panel_c_poolEstEnvRichness.pdf",height=4.9,width=4.5)
layout(matrix(c(1:9),ncol=3,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysize){
...
pdf("~/Sample-microplastics/Narrow_panel_a_indEstEnvCoverage.pdf",height=3.2,width=8)
layout(matrix(c(1:10,11,11),ncol=6,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysizeI){
plot(1~1, pch=NA, xlim=c(0,nrowsI[i]),ylim=c(40,102),ylab="",xlab="")
	...
	mtext(data_titlesI[i], side=3, line =0.25, cex=1)
	if(i==7){mtext("Estimated %coverage, material types",line=2.5,side=2,adj=1,cex=0.8)}
	if(i==1){mtext("Particles chemically identified",line=2.5,side=1,adj=-0.05)}
}
plot(1~1, pch=NA, xlim=c(0,nrowsI[i]),ylim=c(0,1),ylab="",xlab="",bty="n",xaxt="n",yaxt="n")
...
dev.off()
#
pdf("~/Sample-microplastics/Narrow_panel_b_equalEstEnvCoverage.pdf",height=4.9,width=4.5)
layout(matrix(c(1:9),ncol=3,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysize){
plot(1~1, pch=NA, xlim=c(0,nrows[i]),ylim=c(40,102),ylab="",xlab="")
	...
	abline(h=100,lty=2,lwd=1.5)
	mtext(data_titles[i], side=3, line =0.25, cex=1)
	if(i==5){mtext("Estimated %coverage, material types",line=2.5,side=2)}
	if(i==8){mtext("Particles chemically identified",line=2.5,side=1)}
}
dev.off()
#
pdf("~/Sample-microplastics/Narrow_panel_c_poolEstEnvCoverage.pdf",height=4.9,width=4.5)
layout(matrix(c(1:9),ncol=3,byrow=T))
par(mar=c(2,1,2,1))
par(oma=c(2,3,1,0))
for(i in bysize){
