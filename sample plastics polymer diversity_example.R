...
write.csv(DescStats,"~/Microplastics-subsampling/descriptive stats_narrow_onejar_example.csv")
...
Rand <- 	lapply(1:length(datasets), function(x) lapply(1:(nrows[x]), function(smplsz) as.data.frame(t(sapply(1:200, function(z) sampleplastics(datasets[[x]],smplsz,3,5,random=TRUE) ) )) ))# sampling randomly, note the scoreable type column number is not used and can be any number
save(Rand,file="~/Microplastics-subsampling/Rand_i_example.Rdata")
...
save(Shp,file="~/Microplastics-subsampling/Shp_i_example.Rdata")
...
save(Col,file="~/Microplastics-subsampling/Col_i_example.Rdata")
...
save(ShpCol,file="~/Microplastics-subsampling/ShpCol_i_example.Rdata")
...
write.csv(minimums,"~/Microplastics-subsampling/minimums_SummedErrorLessThan20_narrow_onejar_example.csv",row.names=F)
#
...
write.csv(minimumsR,"~/Microplastics-subsampling/minimums_EstSampleRichnessGreaterThan80_narrow_onejar_example.csv",row.names=F)
...
write.csv(minimumsAE,"~/Microplastics-subsampling/minimums_EstEnvRichnessGreaterThan80_narrow_onejar_example.csv",row.names=F)
...
write.csv(minimumsC,"~/Microplastics-subsampling/minimums_EstEnvCoverageGreaterThan80_narrow_onejar_example.csv",row.names=F)
...
pdf("~/Microplastics-subsampling/SummedError_narrow_onejar_example.pdf",height=3,width=6)
layout(matrix(1:3,ncol=3,byrow=T))
par(oma=c(1,4,1,2))
par(mar=c(3,0,2,0))
for(i in 1:3){ #datasets are: Primpke 3 raingarden 1, Lusher sediment
